...
class RolertSignal<args extends unknown[] | unknown = void> {
  ...
  private static readonly gameSignals = new Map<number, RolertSignal<unknown>>()
  ...
  Alert(...args: params<args>) {
    ...
    xpcall(
      () => {
        for (const middleware of this.middlewares) if (!middleware(...args)) return

        for (const attachedSignal of this.attachedSignals) attachedSignal.Alert(...args)
        for (const [id, element] of this.connections) {
          if (typeIs(element, 'RBXScriptConnection')) continue
          if (element.state === ConnectionState.Dead) this.killConnection(id)
  ...
  private registerConnection(conn: RolertConnection | RBXScriptConnection) {
    if (typeIs(conn, 'RBXScriptConnection')) this.connections.set(RolertSignal.connectionIdCount++, conn)
    else {
  ...
  attach(otherAlert: RolertSignal<args>) {
    ...
    for (const otherSignalAttached of otherAlert.attachedSignals) {
      if (otherSignalAttached.id === this.id) return warn('already attached to it ')
    }
  ...
  detach(otherAlert: RolertSignal<unknown>) {
    this.attachedSignals = this.attachedSignals.filter((alert) => alert.id === otherAlert.id)
  }
   ...
   */
  createBindToRBXSignal<T extends Callback>(rbxSignal: RBXScriptSignal<T>, persist: boolean, argumentArray: params<args>, name?: string) {
  ...
  connect(callback: callback<args>, name?: string) {
    ...
    const conn = new RolertConnection(ConnectionType.Connect, name, id, (...args: params<args>) => callback(...args))
  ...
  once(callback: callback<args>, name?: string) {
    if (!this.alive) return warn('signal is dispatched')
    const id = RolertSignal.connectionIdCount++
    const conn = new RolertConnection(ConnectionType.Once, name, id, (...params: params<args>) => {
   ...
   */
  expire(duration: number, onSuccess: callback<args>, onExpire?: () => void, _yield: boolean = false, name?: string) {
    if (!this.alive) return warn('signal is dispatched')
    const id = RolertSignal.connectionIdCount++
    let T = tick()
    const conn = new RolertConnection(ConnectionType.Once, name, id, (...params: params<args>) => {
